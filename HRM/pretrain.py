...
class PretrainConfig(pydantic.BaseModel):
    ...
    overfit_one_batch: bool = False  # Repeat the first train batch for quick sanity check
    ...
    with torch.inference_mode():
        ...
        for set_name, batch, global_batch_size in eval_loader:
            ...
            if metric_values is None:
                ...
                metric_values = torch.zeros((len(set_ids), len(metrics.values())), dtype=torch.float32, device=metric_device)
...
def launch(hydra_config: DictConfig):
    ...
    if RANK == 0:
        try:
            ...
            try:
                ...
                pass
            optimizer_keys = [k for k in (
                "lr", "lr_min_ratio", "lr_warmup_steps",
                "beta1", "beta2", "weight_decay",
                "puzzle_emb_lr", "puzzle_emb_weight_decay"
            ) if k in hydra_config]
            eval_keys = [k for k in (
                "eval_interval", "checkpoint_every_eval", "final_eval", "eval_save_outputs"
            ) if k in hydra_config]
            regularizer_keys = [k for k in ("weight_decay", "puzzle_emb_weight_decay") if k in hydra_config]
    ...
    if RANK == 0:
        ...
        if config.enable_wandb:
            try:
                ...
                _wandb.log({"num_params": sum(x.numel() for x in train_state.model.parameters())}, step=0)
            except Exception:
                pass
    ...
    if config.overfit_one_batch:
        ...
        while train_state.step < train_state.total_steps:
            metrics = train_batch(config, train_state, first_batch, first_global_batch_size, rank=RANK, world_size=WORLD_SIZE)
...
if __name__ == "__main__":
    ...
    if quick_flag:
        ...
        has_bs_override = any(
            s.startswith("global_batch_size=") for s in overrides
        )
