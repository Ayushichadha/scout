...
class CastedSparseEmbedding(nn.Module):
    def __init__(
        ...
        # nn.Parameter ensures local_weights is a leaf tensor in PyTorch 2.10+
        # (nn.Buffer with requires_grad=True no longer produces a leaf in 2.10).
        self.local_weights = nn.Parameter(torch.zeros(batch_size, embedding_dim))
        # Non-trainable parameter: leaf tensor, but optimizer skips gradient update.
        self.local_ids = nn.Parameter(
            torch.zeros(batch_size, dtype=torch.int32), requires_grad=False
        )
